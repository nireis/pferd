...
{	
	...
	//We check if the camera is inside of the Bounding Box(BB)
	bool insideBB = false;
	float l = length(objPos - camPos);
	//dir = -dir;
	...
	{	
		...
		vec4 texColor = access3DTex((camPos+(dir*step)), tex3D_dim.z);

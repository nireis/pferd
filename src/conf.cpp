...
void readConf(string fname, conf* confstr){
	string buffer;
	ifstream f;
...
void parseLine(string* line, conf* confstr){
	/*
	...
	if(key == "showVis"){
		if(value.size() == 1){
			confstr->showVis = toBool(&value[0]);
		}
		else{
		...
		if(value.size() == 1){
			confstr->playSound = toBool(&value[0]);
		}
		else{

...
const int Graph2::BinID = 45;
const std::string Graph2::dateiendung = "grp";
...
bool Graph2::setGraph(std::string graphdata, bool write_binary){
	if( is_set )
		return false;
...
int Graph2::checkDataFile(std::string graphdata){
	fstream checkfile;
	std::string tmp =  graphdata;
	std::size_t p;
	p = tmp.find_last_of(".");
...
bool Graph2::parseTextGraphFile(std::string graphdata){
	parser p = parser(graphdata);
...
ED Graph2::getEdgeData(unsigned int id){
	if(id < edge_count)
		return edge_data[id];
 ...
 * keine Indexprüfung !
 */
unsigned int Graph2::getLowerOutEdgeBound(unsigned int id){
	return nodes_out_offs[id] ;
}
unsigned int Graph2::getUpperOutEdgeBound(unsigned int id){
	return nodes_out_offs[id+1] ;
}
unsigned int Graph2::getLowerInEdgeBound(unsigned int id){
	return nodes_in_offs[id] ;
}
unsigned int Graph2::getUpperInEdgeBound(unsigned int id){
	return nodes_in_offs[id+1] ;
}
